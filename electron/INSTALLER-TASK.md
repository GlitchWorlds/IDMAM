# IDMM Windows Installer - Build Task

## Context
IDMM (Internet Download Manager Max) is complete at D:\IDMM\ with 3 components:
...
1. **electron-builder** config in electron/package.json:
   ...
   - Install to: C:\Program Files\IDMM\
...
2. **NSIS installer customization** (electron/builder-config.yml or in package.json build):
   ...
   - File associations: .IDMM (optional)
   - Uninstaller that cleans up

3. **Auto-start server**: Electron main.js should automatically start the IDMM API server (port 9977) on app launch - ALREADY DONE in electron/main.js
...
### Steps:
1. Read D:\IDMM\electron\package.json and update the "build" section for electron-builder
2. Create D:\IDMM\electron\assets\icon.ico (convert from png or create)
3. Create D:\IDMM\electron\installer.nsh for NSIS customization (auto-start registry)
4. Run: cd D:\IDMM\electron && npm install --save-dev electron-builder && npx electron-builder --win --config
