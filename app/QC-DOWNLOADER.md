...
| 18 | `_flushChunkState` |  | See **Warning #1** below. Functionally correct but has performance issue. |
...
| 20 | `_doSingleStream` |  | See **Warning #2** below. |
...
| 23 | `_recalcProgress` |  | See **Warning #3** below. |
| 24 | `_checkCompletion` |  | See **Warning #4** below. |
...
###  Warning #1  `_flushChunkState`: DB query inside loop (LOW)
...
###  Warning #2  `_doSingleStream`: wrapper object never sets `exited: true` (MEDIUM)
...
###  Warning #3  `_recalcProgress`: DB write on every progress tick (MEDIUM)
...
###  Warning #4  `_checkCompletion`: redundant `_finalizing` guard (LOW)
