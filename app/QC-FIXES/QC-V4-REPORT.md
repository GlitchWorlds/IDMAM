...
| SAFE_ERROR_PATTERNS missing "URL is required" |  WARNING  Not a security risk (400 validation), but not in pattern list |
| SAFE_ERROR_PATTERNS missing "Settings object required" |  WARNING  Same, low severity |
...
| **SSRF protection does NOT cover redirects** |  **WARNING**  _probeUrl() and chunk-worker follow redirects without re-checking SSRF. A redirect from allowed domain to 127.0.0.1 would bypass SSRF. Medium severity. |
